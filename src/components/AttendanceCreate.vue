  ...
  const isLate = computed(() => session.value?.is_late);
  const isLeftEarly = computed(() => session.value?.is_left_early);
  ...
  async function handleCheckIn() {
    ...
    try {
      ...
      session.value = {};
     await load(fetchsession, session)
      noteInput.value = "";
    } catch (err) {
      ElNotification({
        title: "បរាជ័យ",
  ...
  function querySearch(queryString, cb) {
    ...
    if (isLate.value) {
      source = suggestionlate;
    } else if (isLeftEarly.value) {
      source = suggestionleftearly;
    }
